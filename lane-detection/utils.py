...
def getHistogram(img, minPer=0.1, display=False, region=1):
    ...
    if maxValue == 0: 
        return 0, img if display else 0
    ...
    if display:
        ...
        for x, intensity in enumerate(histValues):
            cv2.line(imgHist, (x, imgHist.shape[0]), (x, imgHist.shape[0] - int(intensity//255//region)), (255, 0, 255), 1)
...
def stackImages(scale, imgArray):
    ...
    if rowsAvailable:
        ...
        for x in range(0, rows):
            if imgArray[x].shape[:2] == imgArray[x].shape[:2]:
